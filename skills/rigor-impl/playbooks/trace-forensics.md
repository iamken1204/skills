...
Distinct from Runtime forensics, which instruments the live process. Here the capture already exists; the artifact is a fixed dataset, read it, don't re-run it. Keep tooling generic: a DevTools or trace parser for cpuprofile and `.json.gz`, a text editor for a spindump, your heap tooling for a heapsnapshot.

1. Identify the format and load it with the right tool. Parse large artifacts in a subagent; keep the reduced finding in the main thread.
...
3. Narrow to the cause. Query for the frames holding the most time and walk the call tree to the hot path. For a leak, follow the retainer chain to a GC root. For a spindump, find the thread stuck on-CPU or blocked and its wait reason.
