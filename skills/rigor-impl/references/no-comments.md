...
Strip comments before review. Spawn a fresh-context comment reviewer, act on accepted findings, and offer structural encodings for claimed constraints. The reviewer is never the context that wrote the code.
...
4. Implement the smallest root-cause fix in scope and remove every named workaround. If the root cause is out of scope, land the smallest in-scope fix and report the rest open. The fix-root-causes and redesign-from-first-principles principles guide intent only. Neither authorizes widening the fence nor fixing instances outside it. Never bolt on symptom guards.
